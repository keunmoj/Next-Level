...
public interface DramaRepository extends JpaRepository<Drama, Integer> {
    ...
    @Query("SELECT new com.ddoya.drama.drama.dto.response.DramaResDto(d.id, d.title, CAST(SUM(dp.hit) AS int)) FROM Drama d INNER JOIN DramaProblem dp ON d.id = dp.drama.id GROUP BY d.id")
