...
export const useSingPopularArtistAllGetHook = (artistId: string) => {
  ...
  const getArtistAll = async () => {
    ...
    let res2;
    if (artistId) {
      res2 = await SingArtistSongGet(artistId);
      setSelectedArtist(artistId);
    } else {
      res2 = await SingArtistSongGet(res1.data.artistList[0].artistId);
